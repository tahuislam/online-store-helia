...
function Header({cartArray,updateCart}) {
    ...
    document.addEventListener('scroll',()=>{
        if(window.scrollY>200){
            setHeaderStyle({backgroundColor:"#000000"}/*{backgroundColor: "rgba(0, 0, 0, 0.356)", backdropFilter: "blur(50px) saturate(200%)"}*/);
        }else{
        ...
        <>
            <header style={headerStyle}>
                ...
                <ul className="rightNav">
                    <li><a href="#">Account <i className="bi bi-chevron-down"></i></a></li>
                    <li><button className="bagBtn" style={{cursor:"pointer"}} onClick={toggleCart}><i className="bi bi-bag"></i></button>
                        <span className="cartDisplayNumber">{cartArray.length}</span>
                    </li>
                </ul>
                <Cart cartStyle={cartStyle} cartArray={cartArray} updateCart={updateCart} toggleCart={toggleCart} />
            </header>
        </>
    )
}
